export const assetManifest = {
  backgrounds: {
    street: "assets/backgrounds/la-paz-playable-map-long.png",
    teleferico: "assets/backgrounds/la-paz-teleferico-map-extended-aligned.png",
  },
  sprites: {
    characterSheet: "assets/sprites/characters-source-green.png",
    heroWalk: "assets/sprites/hero-walk.png",
    blockerWalk: "assets/sprites/blocker-walk.png",
    looterWalk: "assets/sprites/looter-walk.png",
    mallkuWalk: "assets/sprites/mallku-walk.png",
    minerWalk: "assets/sprites/miner-walk.png",
    heroAttack: "assets/sprites/hero-chicote-attack.png",
    foodHelper: "assets/sprites/senora-pollera-helper.png",
    foodHelperWalk: "assets/sprites/senora-pollera-helper-detailed-walk.png",
    policeAlly: "assets/sprites/police-ally.png",
    pacenaKiosk: "assets/sprites/kiosco-paceno-game.png",
  },
};

export const spriteFrames = {
  hero: { x: 35, y: 170, w: 330, h: 520 },
  blocker: { x: 382, y: 188, w: 355, h: 500 },
  looter: { x: 748, y: 206, w: 320, h: 482 },
  mallku: { x: 1068, y: 70, w: 405, h: 620 },
  miner: { x: 1488, y: 38, w: 450, h: 655 },
};
